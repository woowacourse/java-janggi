...
public class JanggiBoardFactory {
        ...
        for (int i = 0; i < 10; i++) {
            for (int j = 0; j < 9; j++) {
                ...
                board.putIfAbsent(position,
                        Piece.of(PieceProperty.of(PieceType.EMPTY_VALUE, Team.NONE), NonMoveableStrategy.of(position)));
            }
        }
    }
    ...
    private void setupUpTeamSoldier() {
        upTeamPiecePositionFactory.upTeamSoldierPositions()
                .forEach(position -> board.putIfAbsent(position,
                        Piece.of(PieceProperty.of(PieceType.SOLDIER, Team.RED),
    ...
    private void setupDownTeamSoldier() {
        downTeamPositionFactory.downTeamSoldierPositions()
                .forEach(position -> board.putIfAbsent(position,
                        Piece.of(PieceProperty.of(PieceType.SOLDIER, Team.GREEN),
                                DownwardSoldierMoveStrategy.of(position))));
    }
    ...
    private void setupUpTeamGuards() {
        upTeamPiecePositionFactory.upTeamGuardPositions()
                .forEach(position -> board.putIfAbsent(position,
                        Piece.of(PieceProperty.of(PieceType.GUARD, Team.RED), GuardMoveStrategy.of(position))));
    }

    private void setupDownTeamGuards() {
        downTeamPositionFactory.downTeamGuardPositions()
                .forEach(position -> board.putIfAbsent(position,
                        Piece.of(PieceProperty.of(PieceType.GUARD, Team.GREEN), GeneralMoveStrategy.of(position))));
    }

    private void setupUpTeamChariots() {
        upTeamPiecePositionFactory.upTeamChariotPositions()
                .forEach(position -> board.putIfAbsent(position,
                        Piece.of(PieceProperty.of(PieceType.CHARIOT, Team.RED), ChariotMoveStrategy.of(position))));
    }

    private void setupDownTeamChariots() {
        downTeamPositionFactory.downTeamChariotPositions()
                .forEach(position -> board.putIfAbsent(position,
                        Piece.of(PieceProperty.of(PieceType.CHARIOT, Team.GREEN), ChariotMoveStrategy.of(position))));
    }

    private void setupUpTeamCannons() {
        upTeamPiecePositionFactory.upTeamCannonPositions()
                .forEach(position -> board.putIfAbsent(position,
                        Piece.of(PieceProperty.of(PieceType.CANNON, Team.RED), CannonMoveStrategy.of(position))));
    }

    private void setupDownTeamCannons() {
        downTeamPositionFactory.downTeamCannonPositions()
                .forEach(position -> board.putIfAbsent(position,
                        Piece.of(PieceProperty.of(PieceType.CANNON, Team.GREEN), CannonMoveStrategy.of(position))));
    }

    private void setupUpTeamHorses() {
        upTeamPiecePositionFactory.upTeamHorsePositions()
                .forEach(position -> board.putIfAbsent(position,
                        Piece.of(PieceProperty.of(PieceType.HORSE, Team.RED), HorseMoveStrategy.of(position))));
    }

    private void setupDownTeamHorses() {
        downTeamPositionFactory.downTeamHorsePositions()
                .forEach(position -> board.putIfAbsent(position,
                        Piece.of(PieceProperty.of(PieceType.HORSE, Team.GREEN), HorseMoveStrategy.of(position))));
    }

    private void setupUpTeamElephants() {
        upTeamPiecePositionFactory.upTeamElephantPositions()
                .forEach(position -> board.putIfAbsent(position,
                        Piece.of(PieceProperty.of(PieceType.ELEPHANT, Team.RED), ElephantMoveStrategy.of(position))));
    }

    private void setupDownTeamElephants() {
        downTeamPositionFactory.downTeamElephantPositions()
                .forEach(position -> board.putIfAbsent(position,
                        Piece.of(PieceProperty.of(PieceType.ELEPHANT, Team.GREEN), ElephantMoveStrategy.of(position))));
    }

    private void setupUpTeamGeneral() {
        Position position = upTeamPiecePositionFactory.upTeamGeneralPosition();
        board.putIfAbsent(position,
                Piece.of(PieceProperty.of(PieceType.GENERAL, Team.RED), GeneralMoveStrategy.of(position)));
    }

    private void setupDownTeamGeneral() {
        Position position = downTeamPositionFactory.downTeamGeneralPosition();
        board.putIfAbsent(position,
                Piece.of(PieceProperty.of(PieceType.GENERAL, Team.GREEN), GeneralMoveStrategy.of(position)));
    }
}
