...
public class BoardFactory {
    ...
    private static void putChoFormation(Map<Position, Piece> board, Formation choFormation) {
        ...
        for (int i = 0; i < formation.size(); i++) {
            ...
            if (type == MA) {
                board.put(new Position(x, 9), new Ma(Team.CHO));
            } else if (type == SANG) {
                board.put(new Position(x, 9), new Sang(Team.CHO));
            }
        }
    }
    ...
    private static void putHanFormation(Map<Position, Piece> board, Formation hanFormation) {
        ...
        for (int i = 0; i < formation.size(); i++) {
            ...
            if (type == MA) {
                board.put(new Position(x, 0), new Ma(Team.HAN));
            } else if (type == SANG) {
                board.put(new Position(x, 0), new Sang(Team.HAN));
            }
        }
    }
