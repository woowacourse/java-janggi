...
public final class OuterElephantLocator implements ElephantLocator {

    @Override
    public Map<Point, Piece> setupElephant(final TeamType teamType) {
        final Map<Point, Piece> locations = new HashMap<>();
        locations.put(new Point(teamType.getInitialRow(), 1), new Elephant(teamType));
    ...
    public Map<Point, Piece> setupHorse(final TeamType teamType) {
        final Map<Point, Piece> locations = new HashMap<>();
        locations.put(new Point(teamType.getInitialRow(), 2), new Horse(teamType));
