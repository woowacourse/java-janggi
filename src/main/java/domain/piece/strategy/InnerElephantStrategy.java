...
public class InnerElephantStrategy implements HorseElephantSetupStrategy {
    private static final List<Piece> HAN_INNER_ELEPHANT_HORSE;
    private static final List<Piece> CHO_INNER_ELEPHANT_HORSE;

    static {
        CHO_INNER_ELEPHANT_HORSE = List.of(
        ...
        HAN_INNER_ELEPHANT_HORSE = List.of(
    ...
    public List<Piece> createElephantHorse(TeamType teamType) {
        if (teamType == TeamType.CHO) {
            return new ArrayList<>(CHO_INNER_ELEPHANT_HORSE);
        }
        return new ArrayList<>(HAN_INNER_ELEPHANT_HORSE);
    }
}
