...
public final class JumpingRule {
    ...
    public List<Intersection> movableDestinations(Side side, List<Path> candidatePaths, AlivePieces alivePieces) {
        return candidatePaths.stream()
                .filter(path -> {
                    ...
                    return screens.size() == 1 && !(screens.getFirst() instanceof Cannon);
                })
                ...
                .filter(destination -> {
                    ...
                    return placedAt.hasDifferentSide(side) && !(placedAt instanceof Cannon);
                })
                .toList();
    }
}
