...
public class InputView {
    ...
    public List<Integer> readMovePiecePosition(Player player) {
        System.out.printf("%s(%s)%s%s님이 움직일 말의 좌표를 입력하세요%n", TextColor.specifyTeamColor(player), player.getTeam(),
                TextColor.exit,
                player.getName());
        return readPosition();
    }
    ...
    public List<Integer> readTargetPosition(Player player) {
        System.out.printf("%s(%s)%s%s님이 이동할 좌표를 입력하세요%n", TextColor.specifyTeamColor(player), player.getTeam(),
                TextColor.exit,
                player.getName());
        return readPosition();
    }
