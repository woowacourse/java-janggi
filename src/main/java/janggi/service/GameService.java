...
public class GameService {
    ...
    public GameService(final Repository repository) {
        this.repository = repository;
    }

    public void movePiece(final Coordinate departure, final Coordinate arrival) {
        final var turn = repository.getTurn();
        final var team = turn.currentTeam();
        final var board = new Board(repository.allPieces());
        ...
        turn.toss();
        repository.updateTurn(turn);
    }

    public boolean isGameOver() {
        final var turn = repository.getTurn();
        final var board = new Board(repository.allPieces());
        return turn.isEnded() || board.isAnyGoongDead();
    }
    ...
    public Team currentTurn() {
        return repository.getTurn().currentTeam();
    }

    public Map<Coordinate, Piece> allPieces() {
        final var coordinatePieceMap = repository.allPieces()
            .stream()
            .collect(toMap(Piece::coordinate, identity()));
