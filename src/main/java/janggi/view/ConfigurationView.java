...
public class ConfigurationView {
    ...
    public boolean readOnlineOrLocal(int maxSeconds) {
        System.out.println("게임 서버에 연결하려면 Y를, 연결하지 않으려면 N을 입력해주세요.");
        System.out.printf("처음 실행의 경우 최대 %s초 가량 소요될 수 있습니다.%n", maxSeconds);
    ...
    public void printConnectingServer() {
        System.out.println("로컬 서버에 연결중입니다..");
        System.out.println(ViewTools.applyColor(
            ViewTools.RED, "! 강제 종료 시 서버 프로세스(docker)가 유지될 수 있습니다 !%n")
        );
    }

    public void printStoppingServer() {
        System.out.println("로컬 서버를 중지하는 중입니다.. (약 2초 소요)");
    }
}
