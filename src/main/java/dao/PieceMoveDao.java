...
//        String sql = "INSERT INTO move (gameId, move_order, from_row, from_col, to_row, to_col) VALUES (?, ?, ?, ?, ?, ?)";
//
...
//                WHERE m.gameId = ?
//                  AND m.move_order = (SELECT MAX(move_order) FROM move WHERE gameId = ?)
//                """;
//
...
//        String sql = "SELECT COALESCE(MAX(move_order), 0) FROM move WHERE gameId = ?";
//
//        try (PreparedStatement ps = TransactionContext.getPreparedStatement(sql)) {
//            ps.setLong(1, gameId);
