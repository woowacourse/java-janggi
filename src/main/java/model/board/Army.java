...
public class Army {
    ...
    private void deploySoldier(Board board, Country country) {
        for (int column : SOLDIER_COLUMN) {
            board.place(Position.of(Row.soldier(country), column), new Piece(country, PieceType.SOLDIER));
        }
    }

    private void deployCannon(Board board, Country country) {
        for (int column : CANNON_COLUMN) {
            board.place(Position.of(Row.cannon(country), column), new Piece(country,PieceType.CANNON));
        }
    }

    private void deployGeneral(Board board, Country country) {
        board.place(Position.of(Row.general(country), GENERAL_COLUMN), new Piece(country,PieceType.GENERAL));
    }

    private void deployGuard(Board board, Country country) {
        for (int column : GUARD_COLUMN) {
            board.place(Position.of(Row.edgePiece(country), column), new Piece(country, PieceType.GUARD));
        }
    }

    private void deployChariot(Board board, Country country) {
        for (int column : CHARIOT_COLUMN) {
            board.place(Position.of(Row.edgePiece(country), column), new Piece(country, PieceType.CHARIOT));
        }
    }
