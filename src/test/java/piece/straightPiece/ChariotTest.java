...
import static position.PositionFixtures.D7;
...
import static position.PositionFixtures.F8;
...
*/
    ...
    void possibleRoutesTest_3() {
        // given
        Piece chariot = new Chariot(HAN, F9);
        Piece horse = new Horse(HAN, F6);
        Board board = new Board(HAN, Set.of(chariot, horse));

        // when
        Set<Position> positions = chariot.possibleRoutes(board);

        // then
        assertThat(positions).containsOnly(A9, B9, C9, D9, E9, G9, H9, I9, F8, F7, E8, D7);
    }
}
