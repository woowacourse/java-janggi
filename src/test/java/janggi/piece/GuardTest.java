...
class GuardTest {
    ...
    @ParameterizedTest
    void 초나라_사의_정상적인_움직임을_테스트한다(int column, int row) {
        // given
    ...
    @ParameterizedTest
    void 궁성_밖_목적지로_이동하려는_경우_예외를_발생한다(int column, int row) {
        // given
        Map<Position, Piece> initialBoard = new HashMap<>();
        Position start = createPosition(4, 1);
        Position goal = createPosition(column, row);
        ...
        assertThatThrownBy(() -> board.movePiece(start, goal, Team.GREEN))
                .isInstanceOf(IllegalArgumentException.class)
                .hasMessage("[ERROR] 초나라 사의 목적 지점은 하단부 궁성 내 좌표여야 합니다.");
    }

    @CsvSource(value = {"3:2", "4:2", "5:2"}, delimiterString = ":")
    @ParameterizedTest
    void 한_칸을_초과하여_이동하려는_경우_예외를_발생한다(int column, int row) {
        // given
        Map<Position, Piece> initialBoard = new HashMap<>();
        ...
        assertThatThrownBy(() -> board.movePiece(start, goal, Team.GREEN))
                .isInstanceOf(IllegalArgumentException.class)
                .hasMessage("[ERROR] 사의 이동 규칙에 어긋나는 움직임입니다.");
    }

    @CsvSource(value = {"3:7", "3:8", "3:9", "4:7", "4:9", "5:7", "5:8", "5:9"}, delimiterString = ":")
    @ParameterizedTest
    void 한나라_사의_정상적인_움직임을_테스트한다(int column, int row) {
        // given
        Map<Position, Piece> initialBoard = new HashMap<>();
    ...
    void 한나라_궁성_밖_목적지로_이동하려는_경우_예외를_발생한다(int column, int row) {
        ...
        assertThatThrownBy(() -> board.movePiece(start, goal, Team.RED))
                .isInstanceOf(IllegalArgumentException.class)
                .hasMessage("[ERROR] 한나라 사의 목적 지점은 상단부 궁성 내 좌표여야 합니다.");
    }

    @CsvSource(value = {"3:7", "4:7", "5:7"}, delimiterString = ":")
    @ParameterizedTest
    void 한나라_사가_한_칸을_초과하여_이동하려는_경우_예외를_발생한다(int column, int row) {
        // given
        Map<Position, Piece> initialBoard = new HashMap<>();
