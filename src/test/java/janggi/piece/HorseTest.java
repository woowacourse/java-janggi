...
class HorseTest {
    ...
    @DisplayName("마가 가진 규칙을 적용할 수 없으면 예외가 발생한다.")
    @Test
    void testCannotMove() {
        // given
        ...
        // when
        // then
        assertThatThrownBy(() -> horse.validateMove(start, end, null))
                .isInstanceOf(IllegalArgumentException.class)
                .hasMessage("[ERROR] 기물의 규칙에 맞지 않는 움직임입니다.");
    }

    @DisplayName("마의 중간 경로에 다른 기물이 있으면 예외가 발생한다.")
    @Test
    void testCannotMoveThrough() {
        ...
        // then
        assertThatThrownBy(
                () -> horse.validateMove(start, end, Map.of(new Position(Row.SIX, Column.FIVE), Horse.of(Team.HAN))))
                .isInstanceOf(IllegalArgumentException.class)
