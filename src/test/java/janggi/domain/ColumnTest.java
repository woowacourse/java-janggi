package janggi.domain;


import janggi.exception.ColumnOutOfRangeException;
import org.assertj.core.api.Assertions;
import org.junit.jupiter.api.Test;

class ColumnTest {
    @Test
    void 장기판의_y좌표_범위를_벗어나면_예외가_발생한다() {
        Assertions.assertThatThrownBy(() -> Column.of(11)).isInstanceOf(ColumnOutOfRangeException.class);
        Assertions.assertThatThrownBy(() -> Column.of(-1)).isInstanceOf(ColumnOutOfRangeException.class);
    }

    @Test
    void 장기판의_y좌표_범위_내라면_예외가_발생하지_않는다() {
        Column column1 = Column.of(1);
        Column column2 = Column.of(10);

        Assertions.assertThat(column1.getColumn()).isEqualTo(1);
        Assertions.assertThat(column2.getColumn()).isEqualTo(10);
    }

    @Test
    void 동등성_테스트() {
        Column column = Column.of(1);

        Assertions.assertThat(column).isEqualTo(Column.of(1));
    }
}
