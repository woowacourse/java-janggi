...
class SlideMoveStrategyTest {
    ...
    @Test
    void determineDestinations_NoObstacles_ReturnsAllPaths() {
        ...
        List<Position> destinations = strategy.determineDestinations(paths, boardState, chariot);
    ...
    @Test
    void determineDestinations_EnemyObstacle_StopsAtEnemy() {
        SlideMoveStrategy strategy = new SlideMoveStrategy();
        Position current = new Position(5, 5);
        Piece chariot = new Piece(Side.CHO, PieceType.CHARIOT, "0");
        ...
        Paths paths = strategy.findMovablePaths(current, EnumSet.of(Direction.N));
        Map<Position, Piece> boardState = new HashMap<>();
        boardState.put(new Position(3, 5), new Piece(Side.HAN, PieceType.HAN_SOLDIER, "0"));
