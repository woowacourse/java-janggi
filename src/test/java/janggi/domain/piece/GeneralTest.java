...
class GeneralTest {
    ...
    @Test
    void isGeneralTest() {
        general = new General(new Position(3, 2), RED);
        Assertions.assertThat(general.isSameType(PieceType.GENERAL)).isTrue();
    }

    @DisplayName("왕 기물이 궁성 위치에서(3, 2)이 올바른 루트를 계산하는지 확인한다. (궁성 밖으로 나가는 경우도 포함)")
    @Test
    ...
    @DisplayName("왕 기물이 궁성 위치에서(4, 1)이 올바른 루트를 계산하는지 확인한다. (궁성 밖으로 나가는 경우도 포함)")
    @Test
