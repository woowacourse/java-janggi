...
class GungTest {
    ...
    void 궁은_한_칸_초과하여_움직일_수_없다() {
        ...
        assertThatThrownBy(() -> gung.makePath(currentPosition, arrivalPosition))
                ...
                .hasMessageContaining("[ERROR] 궁은 이어진 선을 따라 한 칸만 이동할 수 있습니다.");
    }
    ...
    void 궁은_궁성_안에서_간선을_따라_대각선으로_1칸_이동한다(final int currentY, final int currentX, final int arrivalY,
                                        final int arrivalX) {
        // Given
        Position currentPosition = new Position(currentY, currentX);
        Position arrivalPosition = new Position(arrivalY, arrivalX);
    ...
    void 궁은_궁성_밖으로_벗어날_수_없다() {
        ...
        assertThatThrownBy(() -> gung.makePath(currentPosition, arrivalPosition))
                .isInstanceOf(IllegalArgumentException.class)
                .hasMessageContaining("[ERROR] 궁은 궁성 밖을 벗어날 수 없습니다.");
    }

    @Test
    void 궁은_궁성_안에서_간선이_이어지지_않았다면_이동할_수_없다() {
        // Given
        final int currentY = 8;
        final int currentX = 5;
