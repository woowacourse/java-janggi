...
@ReplaceUnderBar
class GuardPalaceMovementStrategyTest {

    private static final GuardPalaceMovementStrategy GUARD_PALACE_MOVEMENT_STRATEGY =
    ...
    private static Stream<Arguments> 가드는_궁성_내에서_이동할_수_있다_테스트_케이스() {
        return Stream.of(
    ...
    @ParameterizedTest
    @MethodSource("가드는_궁성_내에서_이동할_수_있다_테스트_케이스")
    void 가드는_궁성_내에서_이동할_수_있다(Position origin, Side side, Position destination, boolean expected) {
        Pieces map = makePieces();

        assertThat(GUARD_PALACE_MOVEMENT_STRATEGY.isMovableInPalace(map, origin, side, destination))
            .isEqualTo(expected);
    }
