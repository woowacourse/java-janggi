...
class HorseStrategyTest {
    ...
    void 직선_1칸_이동_후_대각선_1칸_이동한다(Position source, Position destination) {
        assertThatNoException().isThrownBy(() ->
                moveStrategy.validate(source, destination, CampType.HAN, new Board(Map.of()), PieceRule.HORSE));
    }
    ...
    void 행마법_대로_움직이지_않으면_예외가_발생한다(Position source, Position destination) {
        assertThatThrownBy(() -> moveStrategy.validate(source, destination, CampType.CHO, new Board(Map.of()), PieceRule.HORSE))
    ...
    void 이동하려는_경로에_기물이_존재하면_예외가_발생한다() {
        ...
        assertThatThrownBy(() -> moveStrategy.validate(source, destination, CampType.HAN, blockingBoard, PieceRule.HORSE))
                .isInstanceOf(IllegalArgumentException.class)
