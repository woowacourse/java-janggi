...
public class MultiStepStraightStrategyTest {
    ...
    class exception {
        ...
        void 차와_포는_궁성_대각선이_연결되지_않은_경로로_이동할_수_없다(Position source, Position destination, Camp camp) {
        ...
        private static Stream<Arguments> exceptionDiagonalPathOutOfPalace() {
            return Stream.of(
                    Arguments.of(new Position(0, 3), new Position(3, 6), Camp.CHO),
                    Arguments.of(new Position(3, 6), new Position(0, 3), Camp.CHO),
                    Arguments.of(new Position(7, 3), new Position(6, 2), Camp.HAN),
                    Arguments.of(new Position(6, 2), new Position(7, 3), Camp.HAN)
            );
        }

        @ParameterizedTest
        @MethodSource("exceptionDiagonalPathOutOfPalace")
        void 차와_포는_궁성_영역을_벗어나는_대각선으로_이동할_수_없다(Position source, Position destination, Camp camp) {
            assertThatThrownBy(() -> strategy.findPath(source, destination, camp))
                    .isInstanceOf(IllegalArgumentException.class)
