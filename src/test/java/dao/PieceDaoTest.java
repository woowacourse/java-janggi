...
public class PieceDaoTest {
    private final String GAME_ROOM_NAME = "room1";

    private final ConnectionGenerator connectionGenerator = new EmbeddedH2ConnectionGenerator();
    private Connection testConnection;
    private PieceDao pieceDAO = new PieceDaoImpl();
    private GameRoomDao gameRoomDAO = new GameRoomDaoImpl();

    @BeforeEach
    void setup() throws SQLException {
        testConnection = connectionGenerator.createConnection();
        testConnection.setAutoCommit(false);
        gameRoomDAO.insert(testConnection, new GameRoomDto(null, GAME_ROOM_NAME, Team.CHO));
    }
    ...
    void row를_추가한다() {
        // given
        final PieceDto piece = new PieceDto(null, 1, 1, PieceType.CHA, Team.HAN, GAME_ROOM_NAME);
    ...
    void 방_이름으로_모든_row를_조회한다() {
        // given
        final PieceDto piece1 = new PieceDto(null, 1, 1, PieceType.CHA, Team.HAN, GAME_ROOM_NAME);
        final PieceDto piece2 = new PieceDto(null, 1, 2, PieceType.CHA, Team.HAN, GAME_ROOM_NAME);
        pieceDAO.insert(testConnection, piece1);
        pieceDAO.insert(testConnection, piece2);

        // when
        final List<PieceDto> pieces = pieceDAO.findByGameRoomName(testConnection, GAME_ROOM_NAME);
    ...
    void 방_이름과_좌표로_조회하여_좌표_정보를_업데이트한다() {
        ...
        final PieceDto piece = new PieceDto(null, oldPoint.row(), oldPoint.column(),
    ...
    void 방_이름과_좌표로_조회하여_row를_삭제한다() {
        ...
        final PieceDto piece = new PieceDto(null, point.row(), point.column(),
                PieceType.CHA, Team.HAN, GAME_ROOM_NAME);
        pieceDAO.insert(testConnection, piece);
