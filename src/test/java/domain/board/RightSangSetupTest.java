...
class RightSangSetupTest {
    ...
    @Test
    void 한나라진영이_오른상차림으로_초기화한다() {
        // given
        SangSetup rightSangSetup = new RightSangSetup();
        // when
        Board board = rightSangSetup.initialize(Side.HAN);
        // then
        assertThat(board.pieces()).isEqualTo(expectedHanBoard());
    }

    @Test
    void 초나라진영이_오른상차림으로_초기화한다() {
        // given
        SangSetup rightSangSetup = new RightSangSetup();
        // when
        Board board = rightSangSetup.initialize(Side.CHO);
        // then
    ...
    private static void putChoPieces(Map<Position, Piece> board) {
        ...
        board.put(new Position(0, 6), new Ma(Side.CHO));
        board.put(new Position(0, 7), new Sang(Side.CHO));
    ...
    private static void putHanPieces(Map<Position, Piece> board) {
        ...
        board.put(new Position(9, 1), new Sang(Side.HAN));
        board.put(new Position(9, 2), new Ma(Side.HAN));
