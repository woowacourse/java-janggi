...
class OuterSangSetupTest {
    ...
    void 한나라진영이_바깥상차림으로_초기화한다() {
        // given
        SangSetup outerSangSetup = new OuterSangSetup();
        // when
        Board board = outerSangSetup.initialize(Side.HAN);
        // then
        assertThat(board.pieces()).isEqualTo(expectedHanBoard());
    }

    @Test
    void 초나라진영이_바깥상차림으로_초기화한다() {
        // given
        SangSetup outerSangSetup = new OuterSangSetup();
        // when
        Board board = outerSangSetup.initialize(Side.CHO);
        // then
