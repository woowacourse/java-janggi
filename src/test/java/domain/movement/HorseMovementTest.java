...
class HorseMovementTest {
    ...
    void fromCenterReachablePositionsAreCorrect() {
        ...
        assertThat(canReach(paths, pos(Col.F, Row.TWO))).isTrue();
        assertThat(canReach(paths, pos(Col.D, Row.SIX))).isTrue();
        assertThat(canReach(paths, pos(Col.F, Row.SIX))).isTrue();
        assertThat(canReach(paths, pos(Col.C, Row.THREE))).isTrue();
        assertThat(canReach(paths, pos(Col.C, Row.FIVE))).isTrue();
        assertThat(canReach(paths, pos(Col.G, Row.THREE))).isTrue();
        assertThat(canReach(paths, pos(Col.G, Row.FIVE))).isTrue();
    }
