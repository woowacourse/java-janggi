...
class ByeongTest {
    ...
    void 아군_기물을_잡으면_예외가_발생한다(Piece target) {
        Byeong testByeong = new Byeong(new ByeongMoveStrategy(), new NormalMovementPolicy(), Team.CHO);
        Assertions.assertThatThrownBy(() -> testByeong.capture(target)).isInstanceOf(IllegalArgumentException.class);
    ...
    void 상대_기물은_잡을_수_있다(Piece target) {
        Byeong testByeong = new Byeong(new ByeongMoveStrategy(), new NormalMovementPolicy(), Team.CHO);
        Assertions.assertThatNoException().isThrownBy((() -> testByeong.capture(target)));
