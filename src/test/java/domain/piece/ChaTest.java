...
class ChaTest {
    ...
    void 아군_기물을_잡으면_예외가_발생한다(Piece target) {
        Assertions.assertThatThrownBy(() -> CHA_OF_CHO.capture(target)).isInstanceOf(IllegalArgumentException.class);
    ...
    void 상대_기물은_잡을_수_있다(Piece target) {
        Assertions.assertThatNoException().isThrownBy((() -> CHA_OF_CHO.capture(target)));
    }
