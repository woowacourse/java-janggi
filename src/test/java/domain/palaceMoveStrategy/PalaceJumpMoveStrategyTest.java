...
public class PalaceJumpMoveStrategyTest {
    ...
    @Test
    @DisplayName("포는 궁성에서 대각선으로 기물을 넘어 상대 기물을 먹을 수 있다")
    void move_over_piece_and_capture_opponent_piece() {
        // given
        Position from = new Position(1, 4);
        Position screen = new Position(2, 5);
    ...
    @Test
    @DisplayName("포는 궁성에서 포를 넘을 수 없다")
    ...
    @Test
    @DisplayName("포는 궁성에서 넘을 기물이 없으면 이동할 수 없다")
    ...
    @Test
    @DisplayName("포는 궁성 이동 규칙 기준 대각선으로만 이동 가능하다")
    void cannot_move_diagonally() {
        ...
        Map<Position, Place> board = new HashMap<>();
        board.put(from, new Cannon(Side.CHO, moveStrategy, palaceMoveStrategy));
        board.put(new Position(2, 4), new Soldier(Side.CHO, new SoldierMoveStrategy(Side.CHO),
                new PalaceSoldierMoveStrategy(Side.CHO)));
