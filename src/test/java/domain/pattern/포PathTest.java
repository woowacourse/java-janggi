...
public class 포PathTest {
    ...
    void 포의_이동_전_후_위치를_입력받으면_알맞은_경로를_찾을_수_있다(int afterRow, int afterColumn, List<Pattern> path) {
        // given
        int beforeRow = 10;
        int beforeColumn = 1;

        // when
        List<Pattern> 포path = 포Path.getPath(beforeRow, beforeColumn, afterRow, afterColumn);
    ...
    static Stream<Arguments> provide포Path() {
        return Stream.of(
                Arguments.of(5, 1, List.of(포Path.UP.getPattern(), 포Path.UP.getPattern(), 포Path.UP.getPattern(),
    ...
    void 포의_이동_전_후_위치가_알맞지_않으면_예외를_발생시킨다() {
        ...
        // when & then
        Assertions.assertThatThrownBy(() -> 포Path.getPath(beforeRow, beforeColumn, afterRow, afterColumn))
