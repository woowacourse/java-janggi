...
import static domain.Fixtures.NINE_THREE;
...
public class ElephantPathTest {
    ...
    void 상의_이동_전_후_위치를_입력받으면_알맞은_경로를_찾을_수_있다(JanggiPosition afterPosition, List<Pattern> path) {
        // when
        List<Pattern> elephantPath = piece.findMovablePath(SIX_FIVE, afterPosition);
    ...
    static Stream<Arguments> provideElephantPath() {
        ...
        return Stream.of(
                ...
                Arguments.of(NINE_SEVEN, pathOfElephant.getPatterns(Direction.DOWN_RIGHT)),
                Arguments.of(NINE_THREE, pathOfElephant.getPatterns(Direction.DOWN_LEFT)),
                Arguments.of(EIGHT_TWO, pathOfElephant.getPatterns(Direction.LEFT_DOWN)),
                Arguments.of(FOUR_TWO, pathOfElephant.getPatterns(Direction.LEFT_UP)),
                Arguments.of(THREE_THREE, pathOfElephant.getPatterns(Direction.UP_LEFT)),
                Arguments.of(THREE_SEVEN, pathOfElephant.getPatterns(Direction.UP_RIGHT))
        );
    }
    ...
    void 상의_이동_전_후_위치가_알맞지_않으면_예외를_발생시킨다() {
        // when & then
        Assertions.assertThatThrownBy(() -> piece.findMovablePath(SIX_FIVE, FOUR_SIX))
