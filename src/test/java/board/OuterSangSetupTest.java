...
class OuterSangSetupTest {
    ...
    void 바깥_상마가_올바른_위치에_초기화된다(Side side) {
        SangSetup setup = new OuterSangSetup();
        Board board = setup.initialize(side);

        Map<Position, Piece> positions = new HashMap<>();
        positions.put(position(side, 0, 1), new Sang(side));
        positions.put(position(side, 0, 2), new Ma(side));
