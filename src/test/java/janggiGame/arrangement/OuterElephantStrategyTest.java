...
class OuterElephantStrategyTest {
    @DisplayName("상을 바깥쪽으로 배치한다.")
    @Test
    void outerElephantStrategy_Test() {
        OuterElephantStrategy strategy = new OuterElephantStrategy();
        ...
        assertInstanceOf(Horse.class, pieces.get(Dot.of(2, 0)));
        assertInstanceOf(Horse.class, pieces.get(Dot.of(6, 0)));

        assertInstanceOf(Elephant.class, pieces.get(Dot.of(1, 0)));
        assertInstanceOf(Elephant.class, pieces.get(Dot.of(7, 0)));
    }
}